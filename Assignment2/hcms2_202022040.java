    ...
    public static void setCurrentTime() {
        ...
        for (Car car : carList) {
            if (car.fromSeoulDistance >= car.endPosition && car.entered && car.selected) { // if (car.positionGap <= 0 && car.entered && car.selected)
                ...
                car.entered = false;
                car.out = true;
                car.toll = tollCalculator.calculateToll(car.toll_distance, car.speed, car.cc);
            }
        }
    }
    ...
    public static void viewAllVehiclesOnHighway() {
        ...
        for (Car car : carList) {
            if (car.entered) {
                onHighway = true;
                ++rowNum;
                System.out.printf("%d. %d %dkm%n",
                        rowNum, car.no, car.fromSeoulDistance);
...
class tollCalculator{
    ...
    public int calculateToll(double distance, int speed, int cc){
        double tempToll = 0.0;
        tempToll = (this.basicCharge + distance * this.distanceCharge) * ((double)speed/100) * ((double)cc/2000);
        toll = (int)tempToll;
        toll = toll - (toll%10);
        return toll;
    }
